...
void game::initialize() {
    ...
    this->logical_device = crow::make_vk_logical_device(
        &this->physical_device, &vk_instance, &this->surface, device_extensions,
        rasterization_queue_index, compute_queue_index);
    ...
    height = std::clamp(
        height, static_cast<int>(surface_capabilities.minImageExtent.height),
    ...
    this->swapchain = crow::make_vk_swapchain(
        ...
        window_extent, color_format, present_mode, rasterization_queue_index,
    ...
    this->cmd_pool_rasterize =
        crow::make_cmd_pool(&this->logical_device, rasterization_queue_index);
...
void game::loop() {
    ...
    //  1) Satisfies the memory
    //  requirements(memoryRequirements.memoryTypeBits).

    //  2) Satifies our own usage requirements. We want to be able to read
    //  the buffer memory from the GPU to the CPU with vkMapMemory, so we
    //  set VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT.

    //  Also, by setting VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, memory
    //  written by the device(GPU) will be easily visible to the host(CPU),
    //  without having to call any extra flushing commands. So mainly for
    //  convenience, we set this flag.
    //  */
    ...
    //     memory_type_index));
    // uint8_t* p_data =
    // static_cast<uint8_t*>(this->logical_device.mapMemory(

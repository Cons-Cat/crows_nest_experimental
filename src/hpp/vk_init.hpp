...
inline auto make_image_views(vk::Device* p_logical_device,
    ...
    for (int i = 0; i < image_views.size(); i++) {
        vk::ImageViewCreateInfo image_view_create_info(
            vk::ImageViewCreateFlags(), (*p_swapchain_images)[i],
            vk::ImageViewType::e2D, format, component_mapping,
            sub_resource_range);
        image_views[i] =
...
inline auto make_framebuffers(
    ...
    for (auto const& image_view : *p_swapchain_image_views) {
        // Mutating the attachments implicitly changes the state of
        // framebuffer_info, since it points to the attachments array. Thus,
        // every instantiation of a framebuffer here points to a different image
        // view in the swapchain.
